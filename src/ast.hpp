...
struct ValueType {
    enum class Kind { Never, Unit, Int, Byte, Double, Bool, Char, String, StringView, Array, Reference, Slice, Box, Vec, TypeParameter, Struct, Enum };
...
inline const ValueType ValueType::Never{ValueType::Kind::Never};
...
inline std::size_t valueTypeAlignment(const ValueType& type) {
    if (type == ValueType::Never) return 1U;
