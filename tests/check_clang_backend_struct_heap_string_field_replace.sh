...
grep -Fq "insertvalue { { ptr, i64 }, i32 }" "${output}.ll"
free_count=$(grep -Fc "call void @free" "${output}.ll")
if [[ ${free_count} -lt 3 ]]; then
    echo "expected field replacements plus final drop to emit at least 3 free calls, got ${free_count}" >&2
    exit 1
fi
