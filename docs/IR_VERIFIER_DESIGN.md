...
| `IrVecPopValue` | cible Vec mutable directe ou projetée | `output : E` | retire le premier élément sans wrapper `Option`, utilisé par `for` consommateur après contrôle de longueur pour préserver l'ordre d'insertion | suite |
